...
## Databricks Certified Exams (Developer, ML Practitioner and Data Scientist)

### Useful Links
#### Databricks Academy login
https://academy.databricks.com/login.html
#### Webassessor login
...
__dbc notebooks - Rahman__<br>
https://www.linkedin.com/pulse/spark-simplified-certification-study-guide-raki-rahman/<br>
https://github.com/mdrakiburrahman/databricks-certification<br>

__dbc notebooks - Singh__<br>
https://medium.com/@sidharth.singh.1894/a-guide-to-learning-apache-spark-and-databricks-spark-certification-16d32c7c41b6<br>
https://github.com/Realsid/databricks-spark-certification<br>

__notes from udemy course__<br>
https://andriymz.github.io/certifications/crt020-feedback/https://sundog-education.com/spark-scala/<br>
    
__linkedin blog with detailed prep notes__<br>
https://www.linkedin.com/pulse/all-you-need-clear-crt020-databricks-certified-associate-kumar<br>

__Medium Blog__<br>
https://towardsdatascience.com/my-10-recommendations-after-getting-the-databricks-certification-for-apache-spark-53cd3690073<br>

__Useful FAQ answers for exam__<br>
...
https://academy.databricks.com/exam/databricks-certified-associate-ml-practitioner
...
#### Loss of internet connection from WSL
Sometimes you can lose connection to internet from WSL. In these cases follow below steps from https://github.com/microsoft/WSL/issues/3438#issuecomment-410518578.

Open windows cmd in admin mode and type these commands<br>
...
Other links that may help are:<br>
https://stackoverflow.com/questions/62314789/no-internet-connection-on-wsl-ubuntu-windows-subsystem-for-linux<br>
https://www.drivereasy.com/knowledge/how-to-fix-media-disconnected-in-windows/#:~:text=If%20you%20see%20Media%20Disconnected,Prompt%2C%20don't%20worry.&text=This%20message%20usually%20occurs%20after,adapter%20or%20the%20network%20configurations
...
### Automatic email updates in python
<br>

#### OPTION 1 Using OAuth and service account with g-suite
This method does not require you to allow less secure apps like the second method below. This is better as Google is phasing out this option and its also more secure. However for this method you need a g-suite account. <br>

The details are listed here: <br>
https://medium.com/lyfepedia/sending-emails-with-gmail-api-and-python-49474e32c81f

##### Useful links
https://developers.google.com/gmail/imap/xoauth2-libraries<br><br>


#### OPTION 2 Allowing less secure apps and using smtplib or OAuth
Warning: not as secure as first option and Google also phasing out ability to allow less secure apps in favour of OAuth approach as used in option 1.<br>
...
https://blog.macuyiko.com/post/2016/how-to-send-html-mails-with-oauth2-and-gmail-in-python.html<br>


#### Import necessary libraries including smtlib
`import smtlib`<br>
...
### Automatic txts from python code

First setup account with twilio as they have python api for sending txts, they dont give Irish numbers but can just get US number and send txts from it still very cheap approx 7c a txt.<br>
Details instructions are in this link:<br>
https://www.fullstackpython.com/blog/send-sms-text-messages-python.html<br>
...
## Useful links
https://towardsdatascience.com/machine-learning-model-development-and-deployment-with-mlflow-and-scikit-learn-pipelines-f658c39e4d58?gi=d0b5137a2e86  
https://www.youtube.com/watch?v=6uZcWqsZ2L4  
https://docs.databricks.com/_static/notebooks/mlflow/mlflow-quick-start-training.html  
https://github.com/amesar/mlflow-spark-summit-2019/tree/master/sklearn  
https://docs.databricks.com/applications/machine-learning/train-model/scikit-learn.html  
...
https://towardsdatascience.com/model-tracking-tools-for-data-science-mlflow-51b681f8452e  
https://mlflow.org/0.2.0/tutorial.html  
https://www.mlflow.org/docs/latest/python_api/mlflow.pyfunc.html  
https://devclass.com/2020/11/16/databricks-mlflow-1_12/  

# Julia


## Useful links
https://pyjulia.readthedocs.io/en/latest/installation.html<Br>
https://github.com/JuliaPy/pyjulia<br>

## Install Julia
First do basic install of the julia language.<br>
Follow instructions on https://julialang.org/downloads/platform/<br>
Essentially:<br>
Download tar from https://julialang.org/downloads/ Take the stable version (currently 1.0.5)<br>
Untar:<br>
`tar -xvzf julia-x.y.z-linux-x86\_64.tar.gz`<br>
Add julia to path by adding below line in .zshrc<br>
`export PATH="$PATH:/home/martin/projects/code_projects/julia_projects/software/julia-1.0.5/bin"`<br>
Check if working by typing `julia` in terminal and should launch the julia REPL.<br>
Add Julia kernel to jupyter by adding IJulia package. First launch julia REPL:<br>
`julia`<br>
Then inside the REPL using Pkg add IJulia<br>
`using Pkg`<br>
Pkg.add("IJulia")<br>
NOTE: The first time you add the package it also build it but if you update something for example a PATH next time you add you need to also build<br>
Pkg.build("IJulia")<br>
You should now have a full julia-1.0.5 kernel in jupyter where you can write native julia code.<br>
Note to remove packages simply use:<br>
Pkg.rm("IJulia")

You could also install PyCall to enable you to import pandas etc into julia but as we are doing the below more optimised install we will not do this now. The below installation is much faster and advised as best practice.<br>

## Pyjulia
I personally think it is good to use python as a main foundation language and then use pyspark and julia as imported lanaguages meaning you can say do ETL from pyspark, fast data analysis in julia and some python ML all in the same notebook.<br>

In order to use julia in python need to use pyjulia library.<br>

Now can do quick installation as below, creating a python venv and installing pyjulia python package there

`mkvirtualenv -p /usr/bin/python3.8 julia_enjuliav`<br>
then inside venv install pyjulia<br>
`pip install julia`<br>

If you then open a python jupyter session and run below code you should be able to import julia.<br>

`import julia`<br>
`julia.install()`<br>
`from julia import Base`<br>

However this will give you below error:

Your Python interpreter is statically linked to libpython<br>

To get around this turn off compliation cache using below lines:<br>
`(https://pyjulia.readthedocs.io/en/latest/troubleshooting.html):<br>
`import julia<br>
`from julia.api import Julia`<br>
`jl = Julia(compiled_modules=False)`<br>
Then can import packages etc:<br>
`from julia import Base`<br>

However this makes julia packages run slow especially the more complex ones and you lose the speed gain from julia. The solution is to build a custom version of python that allows both static and dynamic libraries.

Follow steps at https://www.iram.fr/IRAMFR/GILDAS/doc/html/gildas-python-html/node36.html<br>
Basically
First install below packages as needed for python. IF forget can install them after and just rerun `make` and `make install` inside the python dist folder.<br>
`sudo apt-get install liblzma-dev` <br>
`sudo apt-get install libbz2-dev`<br>

Download python3.7 from http://www.python.org/download/ 3.7 useful as works with spark2.x and spark3.0 still a little buggy with polynote etc)
cd into folder where want to hold julia software files<br>
`tar -xvf Python-3.7.8.tgz`<br>
`cd Python-3.7.8`<br>
`./configure --enable-shared --prefix=/home/martin/projects/code_projects/julia_projects/software/julia_python`<br>
`make`<br>
`make test`<br>
`make install`<br>

Next create a new virtualenv using this custom python<br>
`mkvirtualenv -p /home/martin/projects/code_projects/julia_projects/software/julia_python/bin/python3.7 julia_env`<br>

then inside the environment install IPython and pyjulia
`python -m IPython`<br>
`pip install julia`<br>

Check that this installation is working launch ipython and import julia<br>

Then go inside the julia REPL and install PyCall linked to the custom python.
`julia`<br>
Then inside the REPL<br>
`ENV["PYTHON"]="/home/martin/.virtualenvs/julia_env/bin/python"`<br>
`using Pkg`<br>
`Pkg.add("PYCall")`<br>
This should ensure PyCall is linked to the correct python version allowing dynamic libraries.<Br>

Finally inside the venv add a kernel linked to this venv.<br>
`python -m ipykernel install --user --name=julia_env`<br>

You should now be able to open jupyter choose the julia_env kernel and use julia by below commands<br>
`import julia`<br>
`from julia import Base`<br>
`Base.sind(90)`<br>

You can also change a cell to native jluia using cell magic:<br>
`%load_ext julia.magic`<br>
Then in a cell<br>
`%%julia`<br>
`println("Test")`<br>
